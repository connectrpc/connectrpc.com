...
query parameters. To maximize cache hit rates on shared caches, clients should
order parameters as shown in the **Query-Get** rule above. Servers may respond
with an error if the query parameter is too long to fit in a header or results
in a path or URL that is too long.

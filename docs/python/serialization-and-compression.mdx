...
`codec=proto_json_codec()` during client construction.
...
library's `gzip` at the default compression level. Connect clients default to
sending uncompressed requests and asking for gzipped responses. If you know
that the server supports gzip, you can also compress requests by passing

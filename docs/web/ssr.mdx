...
React Server components use a [more capable serialization logic](https://react.dev/reference/rsc/use-server#serializable-parameters-and-return-values).
In addition to regular JSON-serializable types, it also supports BigInt, Infinity, NaN, and - with caveats - typed

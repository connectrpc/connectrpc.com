...
<Tabs syncKey="python-type">
  <TabItem label="ASGI">
    ...
    class ServerAuthInterceptor:
        ...
        async def on_end(self, token: Token["auth_token"], ctx: RequestContext, error: Exception | None):
  ...
  <TabItem label="WSGI">
    ...
    class ServerAuthInterceptor:
        ...
        def on_end_sync(self, token: Token["auth_token"], ctx: RequestContext, error: Exception | None):
            _auth_token.reset(token)
    ```

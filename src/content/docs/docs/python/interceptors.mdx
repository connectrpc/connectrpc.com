...
service from the [getting started documentation](/docs/python/getting-started/) that says "Goodbye" instead of
...
<Tabs syncKey="python-type">
  <TabItem label="ASGI">
    ...
    ```python
    class GoodbyeInterceptor(UnaryInterceptor):
  ...
  <TabItem label="WSGI">
    ...
    ```python
    class GoodbyeInterceptor(UnaryInterceptorSync):
        def __init__(self, users: list[str]):
            self._users = users
...
<Tabs syncKey="python-type">
  ...
  <TabItem label="WSGI">
    ...
    ```python
    app = GreetServiceWSGIApplication(Greeter(), interceptors=[GoodbyeInterceptor(["user1", "user2"])])
    ```
...
<Tabs syncKey="python-type">
  ...
  <TabItem label="Sync">
    ...
    ```python
    client = GreetServiceClientSync("http://localhost:8000", interceptors=[GoodbyeInterceptor(["user1", "user2"])])
    ```
...
<Tabs syncKey="python-type">
  <TabItem label="ASGI">
    ...
    class ServerAuthInterceptor(MetadataInterceptor):
        def __init__(self, valid_tokens: list[str]):
            self._valid_tokens = valid_tokens

        async def on_start(self, ctx: RequestContext) -> Token[str]:
        ...
        async def on_end(self, token: Token[str], ctx: RequestContext, error: Exception | None):
  ...
  <TabItem label="WSGI">
    ...
    class ServerAuthInterceptor(MetadataInterceptorSync):
        def __init__(self, valid_tokens: list[str]):
            self._valid_tokens = valid_tokens

        def on_start_sync(self, ctx: RequestContext) -> Token[str]:
            authorization = ctx.request_headers().get("authorization")
            if not authorization or not authorization.startswith("Bearer "):
                raise ConnectError(Code.UNAUTHENTICATED, "missing or malformed bearer token")
            token = authorization[len("Bearer "):]
        ...
        def on_end_sync(self, token: Token[str], ctx: RequestContext, error: Exception | None):
            _auth_token.reset(token)
    ```
...
<Tabs syncKey="python-type">
  <TabItem label="Async">
    ...
    class ClientAuthInterceptor(MetadataInterceptor):
  ...
  <TabItem label="Sync">
    ...
    class ClientAuthInterceptor(MetadataInterceptorSync):
...
`MetadataInterceptor` requires both `on_start` and `on_end`, so we provide a no-op `on_end`
even when we only care about populating the outbound headers.

...
Connect has an extensive test suite to verify compatibility with the Connect protocol.
...
Cross-origin resource sharing (CORS) is needed to support web clients on origins other than the server's own.
Connect services are standard ASGI and WSGI applications, so any CORS middleware can be used to
enable CORS.

For example, with an ASGI application using [Starlette's](https://www.starlette.io/)
...
`CORSMiddleware` is used directly as ASGI middleware here — there is no need
to wrap the Connect application in a full `Starlette` app. The middleware
handles CORS preflight (`OPTIONS`) requests itself, returning a 200 response
before they reach the Connect handler.

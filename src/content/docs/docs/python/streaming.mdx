...
All that said, `connectrpc` fully supports client, server, and bidirectional streaming.
...
Let's start by amending the `GreetService` we defined in [Getting Started](/docs/python/getting-started/) to make the Greet method use
client streaming:
...
That's it - metadata interceptors such as our [simple authentication interceptor](/docs/python/interceptors/#metadata-interceptors)

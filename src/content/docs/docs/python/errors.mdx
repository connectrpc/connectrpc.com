...
Error details are `connectrpc.errors.ErrorDetail` classes that can be unpacked into
...
try:
    ...
    for detail in e.details:
        # Returns None if type does not match
        if unpacked := detail.value(Struct):
...
See the [Connect protocol](/docs/protocol#error-end-stream) for a full description
of the error detail fields, including `type`, `value`, and `debug`.

## See also

- [Interceptors](/docs/python/interceptors/) for error transformation and logging
- [Streaming](/docs/python/streaming/) for stream-specific error handling
- [Headers and trailers](/docs/python/headers-and-trailers/) for attaching metadata to errors

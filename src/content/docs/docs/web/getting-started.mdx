...
description: "Build a Connect web client with TypeScript and connect-web — type-safe RPC from the browser without hand-written marshaling."
---
...
```tsx title="src/App.tsx" mark={2-7}
function App() {
  return <>
    <form>
      <input />
...
First, let's capture the user's input and store that as state. That will allow
us to control it easily:

```tsx title="src/App.tsx" mark={2,5}
function App() {
...
```tsx title="src/App.tsx" mark={4-9}
function App() {
  const [inputValue, setInputValue] = useState("");
  return <>
...
```tsx title="src/App.tsx" mark={2-7,11-31}
  const [inputValue, setInputValue] = useState("");
...
```tsx title="src/App.tsx" mark={2-8}
  return <>
    <ol>

...
description: "Build a Connect client for Android with Kotlin — generated, type-safe APIs using OkHttp and Connect-Kotlin."
---
...
supports the [gRPC][grpc], [gRPC-Web][grpc-web], and [Connect](/docs/protocol/)
...
<Image src={SelectActivityScreenshot} width="800" alt="Select empty activity in Android Studio" />
...
:::note
There exist two `build.gradle` files in the project. Use the `build.gradle`
located in `./app/build.gradle` and **not** the one in the root directory.
:::
...
Connect-Kotlin supports the [Connect protocol](/docs/protocol/), the
[gRPC protocol][grpc], and the [gRPC-Web protocol][grpc-web].
Instructions for switching between the three can be found [here](/docs/kotlin/using-clients/).
...
- **Idiomatic, typed APIs.** No more hand-writing REST/JSON endpoints and
  data types. Connect-Kotlin [generates](/docs/kotlin/using-clients/#using-generated-clients)
...
- **Flexibility.** Connect-Kotlin uses `Okhttp`. The library provides
  the option to [swap this out](/docs/kotlin/using-clients/#http-stack).
